...
enum ov5640_mode {
	...
	ov5640_mode_bt656_640_480_FOV2 = 12,
	ov5640_mode_MAX = 12
};
...
static struct reg_value ov5640_bt656_640_480_FOV2[] = {
 // copy of 640x480 VGA
	{0x3c07, 0x08, 0, 0}, {0x3820, 0x41, 0, 0}, {0x3821, 0x07, 0, 0},
	{0x3814, 0x11, 0, 0}, {0x3815, 0x11, 0, 0}, {0x3800, 0x01, 0, 0},
	{0x3801, 0x6C, 0, 0}, {0x3802, 0x01, 0, 0}, {0x3803, 0x1C, 0, 0},
	{0x3804, 0x08, 0, 0}, {0x3805, 0xb3, 0, 0}, {0x3806, 0x06, 0, 0},
	{0x3807, 0x79, 0, 0}, {0x3808, 0x02, 0, 0}, {0x3809, 0x80, 0, 0},
	{0x380a, 0x01, 0, 0}, {0x380b, 0xe0, 0, 0}, {0x380c, 0x08, 0, 0},
	{0x380d, 0x80, 0, 0}, {0x380e, 0x06, 0, 0}, {0x380f, 0x00, 0, 0},
...
static struct reg_value ov5640_init_setting_30fps_BT656[] = {
	...
	{0x3c0b, 0x40, 0x0, 0x0},
#endif
	{0x3a00, 0x7c, 0x0, 0x0}, //night mode 191223
	{0x4001, 0x02, 0x0, 0x0},
	{0x4004, 0x02, 0x0, 0x0},
	{0x4005, 0x1a, 0x0, 0x0},
	{0x5001, 0xa3, 0x0, 0x0},
...
static int ov5640_get_regulators(struct device *dev)
{

	io_regulator = devm_regulator_get(dev, "DOVDD");
	if (IS_ERR(io_regulator)) {
		io_regulator = NULL;
		dev_err(dev, "cannot get io voltage\n");
		return -1;
	}
	...
	if (IS_ERR(analog_regulator)) {
		...
		dev_err(dev, "cannot get analog voltage\n");
		return -1;
	}
...
static int ov5640_download_firmware(struct reg_value *pModeSetting, s32 ArySize)
{
	...
	for (i = 0; i < ArySize; ++i, ++pModeSetting) {
		...
		if (Mask) {
			...
			if (retval < 0) {
				pr_err("ov5640: Failed to read reg 0x%x: 0x%x\n", RegAddr, RegVal);
				goto err;
			}
...
static int ov5640_get_sensor_model(u8 *sensor_model, int n)
{
	...
	for (i = 0; i < n && retval >= 0; i++) {
		...
		if (retval < 0) {
			pr_err("ov5640: failed to read OTP memory 0x%x\n", OV5640_OTP_START_ADDR + i);
			return -1;
		}
	}
...
static int ov5640_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		/* Wait 20ms after reset pulled high before accessing sensor's
		 * registers */
		...
		retval = ov5640_init_mode();
		if (retval < 0) {
			clk_disable_unprepare(ov5640_data.sensor_clk);
			dev_err(dev, "camera ov5640 init failed\n");
			ov5640_power_down(1);
			return retval;
		}

		dev_info(dev, "camera ov5640 powered on\n");
	} else {
		clk_disable(ov5640_data.sensor_clk);
		dev_info(dev, "camera ov5640 powered off\n");
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (retval < 0) {
		dev_err(dev, "Failed to request PWDN pin\n");
		return retval;
	}
	...
	retval = devm_gpio_request_one(dev, rst_gpio, GPIOF_OUT_INIT_LOW, "ov5640_reset");
	if (retval < 0) {
	...
	retval = of_property_read_u32(dev->of_node, "mclk_source", (u32 *) &(ov5640_data.mclk_source));
	if (retval) {
		dev_err(dev, "mclk_source invalid\n");
		return retval;
	}

	retval = of_property_read_u32(dev->of_node, "csi_id", &(ov5640_data.csi));
	if (retval) {
	...
	if (ov5640_get_regulators(&client->dev)) {
